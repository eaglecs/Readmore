...
public class ReadMoreTextView extends TextView implements View.OnClickListener {
    ...
    private CharSequence updateCollapsedText() {
        ...
        return addClickableSpan(trimCollapsedText, trimEndIndex, true);
    }

    private CharSequence updateExpandedText() {
        if (showTrimExpandedText) {
            return addClickableSpan(trimExpandedText, text.length(), false);
        }
        return text;
    }

    private CharSequence addClickableSpan(CharSequence trimText, int endText, boolean isShowEllipize) {

        Spanny spanny = new Spanny(text.subSequence(0, endText), new StyleSpan(Typeface.NORMAL));
        if (isShowEllipize) {
            spanny.append(ELLIPSIZE);
        }
        spanny.append(trimText, new ForegroundColorSpan(colorClickableText));
        return spanny;
    }
